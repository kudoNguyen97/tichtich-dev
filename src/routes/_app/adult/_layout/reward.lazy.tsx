...
const schema = z.object({
  amount: z
    ...
    .min(MIN_AMOUNT, `Số tiền tối thiểu là ${formatVND(MIN_AMOUNT)}đ.`)
    .refine((v) => v % 1000 === 0, "Số tiền phải tròn nghìn (ví dụ: 1.000đ)."),
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
/** Format number to Vietnamese style: 1000 → "1.000" */
function formatVND(n: number) {
  if (!n && n !== 0) return "";
  return n.toLocaleString("de-DE");
}
 
/** Strip all non-digits from a string and return number */
...
function AmountField({ value, onChange, onBlur, error }: any) {
    ...
    return (
      <div className="mb-5">
        <label {...labelProps} className="flex items-center gap-1 text-sm font-bold text-gray-700 mb-2">
          Tặng thưởng <span className="text-red-500">*</span>
        </label>
        ...
        {error && (
          <p {...errorMessageProps} role="alert" className="mt-1.5 text-xs text-red-500">
            {error}
          </p>
        )}
      </div>
    ...
    return (
      <div className="mb-5">
        ...
        <div className="flex items-center justify-between mt-1">
          <p {...errorMessageProps} role="alert" className="text-xs text-red-500 min-h-[16px]">
            {error}
          </p>
          <span
            aria-live="polite"
  ...
  // ─── SuggestionTags ───────────────────────────────────────────────────────────
  function SuggestionTags({ suggestions, currentAmount, onPick }: { suggestions: any, currentAmount: any, onPick: any }) {
    if (!suggestions.length) return null;
    return (
      <div role="group" aria-label="Gợi ý số tiền" className="flex flex-wrap gap-2 -mt-2 mb-5">
...
function RouteComponent() {
    ...
    const [suggestionTagsDismissed, setSuggestionTagsDismissed] = useState(false);
    ...
    const handleAmountChange = (val: any) => {
      ...
      setDisplayValue(isNaN(num) ? "" : formatVND(num));
      if (isNaN(num)) {
        ...
        setValue("amount", num, { shouldValidate: true, shouldDirty: true });
      }
    };
    ...
    return (
        <div className="flex flex-col items-center justify-center px-4">
            <div className="w-full h-[300px]">
                <img className="w-full h-full object-contain" src="/images/reward-adult/pig-family.png" alt="" />
            </div>
            <p className="text-tichtich-black text-base font-bold text-center mt-2">Nhập số tiền để thưởng cho con</p>
      ...
      <TichTichButton
        ...
        <RewardSummaryModal
            ...
            onSend={() => {
                // TODO: gọi API tặng thưởng khi có endpoint
            }}
        />
        </div>
    );
